...
//            feed.enumerated().forEach { (index, imageFeed) in
//                XCTAssertEqual(imageFeed, expectedImage(at: index), "Unexpected result found at \(index)")
//            }
            
            // As we have finite number of feed images then we can also add like below
    ...
    private func getFeedResult(file: StaticString = #filePath, line: UInt = #line) -> FeedLoader.Result? {
        ...
        loader.load { result in
            receivedResult = result as? FeedLoader.Result
