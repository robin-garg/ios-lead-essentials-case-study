...
final class LoadFeedFromCacheUseCaseTests: XCTestCase {
    ...
    func test_load_deliversCachedImagesOnLessThenSevenDaysOldCache() {
        ...
        expect(sut, toCompleteWith: .success(feed.modals), when: {
    ...
    func test_load_deletesCacheFromRetrievalError() {
        let (sut, store) = makeSUT()
    ...
    func test_load_doesNotDeletesCacheOnLessThanSevenDaysOldsCache() {
        let feed = uniqueImageFeed()
        let fixedCurrentDate = Date()
        let lessThanSevenDaysOldTimestamp = fixedCurrentDate.adding(days: -7).adding(seconds: 1)
        let (sut, store) = makeSUT(currentDate: { fixedCurrentDate })
